"use client";


const GOOGLE_REVIEW_URL = "https://g.page/r/Cf2hrUzaa0e_EBM/review";

import { X, ExternalLink, Star } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";

interface GoogleReviewPromptProps {
  open: boolean;
  authorName: string;
  onClose: () => void;
}

export default function GoogleReviewPrompt({
  open,
  authorName,
  onClose,
}: GoogleReviewPromptProps) {
  return (
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 bg-black/70 backdrop-blur-sm"
            onClick={onClose}
          />

          {/* Modal */}
          <motion.div
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.9, y: 20 }}
            transition={{ duration: 0.25 }}
            className="relative z-10 w-full max-w-md bg-[#161b22] border border-emerald-500/20 rounded-xl p-8 shadow-2xl"
          >
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>

            {/* Icon cluster */}
            <div className="flex justify-center mb-6">
              <div className="relative">
                <div className="w-16 h-16 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center justify-center">
                  <Star className="w-8 h-8 fill-amber-400 text-amber-400" />
                </div>
                <div className="absolute -bottom-1 -right-1 w-6 h-6 bg-white rounded-full flex items-center justify-center">
                  {/* Google G */}
                  <svg viewBox="0 0 24 24" className="w-4 h-4" aria-hidden>
                    <path
                      fill="#4285F4"
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    />
                    <path
                      fill="#34A853"
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    />
                    <path
                      fill="#EA4335"
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    />
                  </svg>
                </div>
              </div>
            </div>

            <h2 className="text-xl font-bold text-white text-center mb-2">
              Thank you, {authorName}!
            </h2>
            <p className="text-gray-400 text-center text-sm mb-6 leading-relaxed">
              Your review means a lot to us. Would you also share your
              experience on Google? It helps others discover SUDS Tech and takes
              less than a minute.
            </p>

            <div className="flex flex-col gap-3">
              <a
                href={GOOGLE_REVIEW_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-primary w-full justify-center text-base"
                onClick={onClose}
              >
                Review us on Google
                <ExternalLink className="w-4 h-4" />
              </a>
              <button
                onClick={onClose}
                className="btn-ghost w-full justify-center text-sm"
              >
                Maybe later
              </button>
            </div>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
  );
}
